...
void *aligned_calloc(int size)
{
	...
	return res;
#else
    void *mem = calloc(1, size + ALIGN + sizeof(size_t));
    void **ptr = (void **) (((size_t)(mem) + ALIGN + sizeof(size_t)) & ~(ALIGN - 1));
	ptr[-1] = mem;
	return ptr;
#endif
}
	...
	struct timeval *y)
{
	...
	if (start <= end)
	{
        ...
        result->tv_sec = (uint)(diff / 1000000);
        result->tv_usec = (uint)(diff % 1000000);
	}
	else
	{
		result->tv_sec = 0;
...
static const char *get_stratum_session_id(json_t *val)
{
	...
	if (!arr_val || !json_is_array(arr_val))
		return NULL;
    n = (uint)json_array_size(arr_val);
	for (i = 0; i < n; i++) {
		const char *notify;
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	...
	if (!merkle_arr || !json_is_array(merkle_arr))
		goto out;
    merkle_count = (uint)json_array_size(merkle_arr);
    ...
    for(i = 0; i < (uint)sctx->job.merkle_count; i++)
